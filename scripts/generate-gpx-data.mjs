import { promises as fs } from "node:fs";
import path from "node:path";

const FEET_PER_METER = 3.28084;
const EPSILON = 1e-9;

const root = process.cwd();
const hikesPath = path.join(root, "data", "hikes.json");
const outputPath = path.join(root, "data", "gpx-data.json");

function clamp01(value) {
  if (value < 0) return 0;
  if (value > 1) return 1;
  return value;
}

function downsampleByCount(items, targetCount) {
  if (items.length <= targetCount) return items.slice();
  if (targetCount <= 1) return [items[0]];

  const sampled = [];
  const lastIndex = items.length - 1;
  const step = lastIndex / (targetCount - 1);

  for (let i = 0; i < targetCount; i += 1) {
    const index = Math.round(i * step);
    sampled.push(items[Math.min(index, lastIndex)]);
  }
  return sampled;
}

function parseTrackPoints(xml) {
  const points = [];
  const trkptRegex = /<trkpt\b([^>]*)>([\s\S]*?)<\/trkpt>/gi;

  let trkptMatch = trkptRegex.exec(xml);
  while (trkptMatch) {
    const attrs = trkptMatch[1];
    const body = trkptMatch[2] ?? "";

    const latMatch = /lat\s*=\s*"([^"]+)"/i.exec(attrs);
    const lonMatch = /lon\s*=\s*"([^"]+)"/i.exec(attrs);

    if (latMatch && lonMatch) {
      const lat = Number.parseFloat(latMatch[1]);
      const lon = Number.parseFloat(lonMatch[1]);
      if (Number.isFinite(lat) && Number.isFinite(lon)) {
        const eleMatch = /<ele>([^<]+)<\/ele>/i.exec(body);
        const eleValue = eleMatch ? Number.parseFloat(eleMatch[1]) : 0;
        const ele = Number.isFinite(eleValue) ? eleValue : 0;
        points.push({ lat, lon, ele });
      }
    }

    trkptMatch = trkptRegex.exec(xml);
  }

  return points;
}

function getBounds(points) {
  let minLat = Number.POSITIVE_INFINITY;
  let maxLat = Number.NEGATIVE_INFINITY;
  let minLon = Number.POSITIVE_INFINITY;
  let maxLon = Number.NEGATIVE_INFINITY;

  for (const point of points) {
    if (point.lat < minLat) minLat = point.lat;
    if (point.lat > maxLat) maxLat = point.lat;
    if (point.lon < minLon) minLon = point.lon;
    if (point.lon > maxLon) maxLon = point.lon;
  }

  return { minLat, maxLat, minLon, maxLon };
}

function buildNormalizedTrail(points, bounds) {
  const lonSpan = Math.max(bounds.maxLon - bounds.minLon, EPSILON);
  const latSpan = Math.max(bounds.maxLat - bounds.minLat, EPSILON);

  const normalized = points.map((point) => {
    const x = (point.lon - bounds.minLon) / lonSpan;
    const y = 1 - (point.lat - bounds.minLat) / latSpan;
    return [clamp01(x), clamp01(y)];
  });

  return downsampleByCount(normalized, 150);
}

function buildElevation(points) {
  const sampled = downsampleByCount(points, 80);
  const values = sampled.map((point) => Math.round(point.ele * FEET_PER_METER));

  let min = Number.POSITIVE_INFINITY;
  let max = Number.NEGATIVE_INFINITY;
  for (const value of values) {
    if (value < min) min = value;
    if (value > max) max = value;
  }

  return { values, min, max };
}

function emptyData() {
  return {
    trail: [],
    elevationFt: [],
    rawPoints: [],
    bounds: { minLat: 0, maxLat: 0, minLon: 0, maxLon: 0 },
    elevationMin: 0,
    elevationMax: 0,
  };
}

const hikesRaw = await fs.readFile(hikesPath, "utf8");
const hikes = JSON.parse(hikesRaw);
const output = {};

for (const hike of hikes) {
  try {
    const cleanRelative = String(hike.gpx || "").replace(/^\/+/, "");
    const gpxAbsolute = path.join(root, "public", cleanRelative);
    const xml = await fs.readFile(gpxAbsolute, "utf8");
    const rawPoints = parseTrackPoints(xml);
    if (!rawPoints.length) {
      output[hike.id] = emptyData();
      continue;
    }

    const bounds = getBounds(rawPoints);
    const trail = buildNormalizedTrail(rawPoints, bounds);
    const elevation = buildElevation(rawPoints);

    output[hike.id] = {
      trail,
      elevationFt: elevation.values,
      rawPoints,
      bounds,
      elevationMin: Number.isFinite(elevation.min) ? elevation.min : 0,
      elevationMax: Number.isFinite(elevation.max) ? elevation.max : 0,
    };
  } catch {
    output[hike.id] = emptyData();
  }
}

await fs.writeFile(outputPath, JSON.stringify(output));
console.log(`wrote ${outputPath}`);
